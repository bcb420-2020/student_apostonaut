...
output_hits <-  output_hits[which(output_hits$adj.P.Val < pval),]
# SORT BY DECREASING EXPRESSION VALUE
output_hits <- output_hits[order(output_hits$logFC, decreasing = TRUE),]

testing <- 1
if (testing){
    output_hits[order(output_hits$AveExpr),]
    nrow(output_hits)
    # MAYBE COLOUR THESE DOTS IN THE MA PLOT (THE INCREASE/DECREASE EXPRESSION PLOT)
    output_hits[output_hits$hgnc_symbol == "MAST1",]
    output_hits[output_hits$hgnc_symbol == "VCAN",]
    output_hits[output_hits$hgnc_symbol == "CD58",]
    output_hits[output_hits$hgnc_symbol == "CD200",]
    output_hits[output_hits$hgnc_symbol == "SIGIRR",]
}
# #UPREGULATED
# output_hits.up <- output_hits[which(output_hits$t > 0),]
# length(which(output_hits.up$adj.P.Val < pval))
# up.down="up."
# #cat(noquote(output_hits$hgnc_symbol[which(output_hits$adj.P.Val < pval)]))
# write(output_hits$hgnc_symbol[which(output_hits.up$adj.P.Val < pval)], file = paste0("gene_hits","/",adjust.method,".",pval,".",up.down,"gene_hits.txt") )
# 
# #TEST
# grep("NOTCH3", output_hits.up)
# 
# #DOWNREGULATED
# output_hits.down <- output_hits[which(output_hits$t < 0),]
# length(which(output_hits.up$adj.P.Val < pval))
# up.down="down."
# write(output_hits$hgnc_symbol[which(output_hits.down$adj.P.Val < pval)], file = paste0("gene_hits","/",adjust.method,".",pval,".",up.down,"gene_hits.txt") )
```
