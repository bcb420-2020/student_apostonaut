...
#TABLE FOR COUNTS
title <- "Table 1: counts of gprofiler results"
...
df.counts <- data.frame(names, counts,  counts.filtered, stringsAsFactors = FALSE)#
...
To gain confidence about my top hits for upregulated and downregulated genes, I looked at the 2 highest and lowest with respect to logFC. First I looked at the 2 hits whose expression are most increased, CACNA1G and TPPP. Expression of CACNA1G was highly increased upon treatment of neural induced human adipose tissue-derived mesenchymal stem cells (NI-hADSCs) with HDAC inhibitor[@CACNA1G]. Binding of the TPPP/p25 complex to HDAC6 is know to inhibit its deacetylating function in HeLa cells[@HDAC6_neuro]. Since An increase in both CACNA1G and TPPP are seen in our SAHA-treated sample, this is consistent with HDAC inhibition.\ 
...
I used gprofiler, using the "gprofiler2" R package. I used this meta-database because it has a corresponding R package to fetch data, and we have gained familiarity with it in class. It contains the "GO:BP" dataset (Biological Process), which is the data source which I found most useful for understanding the effect of SAHA on the cell's biology. It returns the most results related to biological function/pathways. I used the p-value of 0.05 as the threshold for gene sets, and subsequently filtered for smaller term size and more stringent p-value. \

<!-- How many genesets were returned with what thresholds?\ -->
The number of genesets for total, upregulated and downregulated gene queries can be seen in Table 1. The column "counts" corresponds to all terms returned with p_value=0.05, and "counts.filtered" correspond to p_value=0.00005, and term.size=500. I used the counts.filtered results to look at higher confidence and more meaningful gene sets. I will refer to the "unfiltered"(p_value=0.05) and "filtered" gprofiler results (p_value=0.00005, and term.size=500) from this point onwards.\
...
Total: When looking at the "total" terms, they seem to relate mostly to cell cycle, autophagy, DNA replication and catabolic processes. "signal transduction by p53 class mediator" is also a top term.\
Upregulated: The top downregulated terms correspond almost exclusively to DNA replication, cell cycle checkpoint, and chromosome segregation gene sets. This seems to have a lot in common with "total" query.
Downregulated: The upregulated terms include vastly different terms than the "total" and "downregulated" filtered queries. It includes a wider variety of functions, including protein localization, lytic vacuole/lysosome, Axonogenesis, wnt signaling, (Ras) GTPase binding, and synapse-related terms.\


<!-- Can you find evidence, i.e. publications, to support some of the results that you see. How does this evidence support your results. -->

## Wnt pathway, Ras and neuronal development effects are seen, not discussed by original paper
...
In the [@WNT_UP] paper, they find a link between HDAC inhibition and neuronal differentiation via the Wnt signalling pathway. In the upregulated filtered gprofiler results, we see terms related to Wnt signalling as well as neuronal differentiation (Table 2, column 2), which is consistent with these results.\

We also see "(RAS) GTPase" terms in the filtered upregulated gene sets. In the [@RAS_HDAC] study, they show that the HDAC inhibitor FK228 suppresses the Ras-MAPK signalling pathway, which supports this finding.\ 
